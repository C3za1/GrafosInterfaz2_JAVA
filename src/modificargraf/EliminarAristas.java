    ...
    public void R_repaint()
    {
        for (int j = 0; j < i; j++) 
        {
            for (int k = 0; k < i; k++) 
            {
                ...
                Dibujar.pintarLinea(Principal.PanelDibujar.getGraphics(),arboles.getCordeX(j),arboles.getCordeY(j), 
        ...
        for (int j = 0; j < i; j++) 
        {
          Dibujar.pintarCirculo(Principal.PanelDibujar.getGraphics(), arboles.getCordeX(j),arboles.getCordeY(j),
          String.valueOf(arboles.getNombre(j)));
        }
    }
 
    public EliminarAristas(Dibujar pinta , Arboles arbole ,int ii, Frame fram) 
    {
       initComponents();
       i=ii;
       this.pintar =pinta;
      ...
      JOptionPane.showMessageDialog(null,"Error.. Faltan datos : ");
        else
        {
            ...
            else
            {
                ...
                n1.setText(null);
                n2.setText(null);
                Principal.PanelDibujar.paint(Principal.PanelDibujar.getGraphics());
